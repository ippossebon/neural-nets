...
class NeuralNetwork(object):
    ...
    def __init__(self, neurons_per_layer, reg_factor, training_data):
    ...
    def initTheta(self):
        ...
        # Inicializa os pesos com valores aleatorios entre 0 e 1
        for i in range(len(self.neurons_per_layer) - 1):
            self.theta[i] = np.random.rand(self.theta[i].shape[0], self.theta[i].shape[1])
    ...
    def backpropagation(self, thetas, should_print=True):
        ...
        for i in range(len(self.training_data)):
            ...
            classification = []
            for key in self.training_data[i].classification:
                classification.append(self.training_data[i].classification[key])
            y.append(classification)

            f_x, activations = self.forwardPropagation(instance=self.training_data[i], thetas=thetas, should_print=should_print)
            f_theta.append(f_x)

            should_print and print('\tSaida predita para o exemplo {0} = {1}'.format(i, f_x))
            should_print and print('\tSaida esperada para o exemplo {0} = {1}'.format(i, classification))

            # Calcula o custo
            j_i = self.cost(f_x, classification)
            J_vector.append(j_i)

            should_print and print('\tJ do exemplo {0} = {1}'.format(i+1, j_i))

            # Calcula delta da camada de saída
            j_i = f_x - classification
            ...
            for k in range(last_layer_index-1, 0, -1):
                ...
                activation_copy = np.array(activations[k])
            ...
            for k in range(last_layer_index-1, -1, -1):
                ...
                activation_matrix = np.matrix(activations[k])
        ...
        for k in range(last_layer_index-1, -1, -1):
            # θ(l=k) = θ(l=k) - α .* D(l=k)
            new_thetas[k] = np.asarray(np.multiply(thetas[k], self.learning_rate * D_matrix[k]))
    ...
    def forwardPropagation(self, instance, thetas, should_print=True):
        ...
        for i in range(len(self.neurons_per_layer)):
            ...
            # camada + 1 (bias)
            activations.append(np.zeros(shape=(self.neurons_per_layer[i]+1, 1)))


        activations[0] = bias + instance.attributes
        ...
        for k in range(1, self.num_layers-1):
            ...
            #z[k] = self.theta[k-1] * activations[k-1]
            z[k] = np.dot(thetas[k-1], activations[k-1])
            aux = g_vector_function(z[k])
            activations[k] = np.insert(aux, 0, 1, axis=0)

        # Ativação do neurônio da camada de saída
        k = self.num_layers-1
        z[k] = np.dot(thetas[self.num_layers-2], activations[self.num_layers-2])
        activations[k] = g_vector_function(z[k])

        should_print and print('\t\tVetor a =')
        for i in range(len(activations)):
            should_print and print('\t\t\t{0}'.format(list(activations[i])))
        ...
        should_print and print('\t\tf(x) = {0} \n'.format(activations[k]))

        # Predição final
        return activations[k], activations
    ...
    def runNetwork(self, max_iter):
        ...
        self.initTheta()
        new_thetas = self.theta

        for i in range(max_iter):
            j = new_j
            new_j, new_thetas = self.backpropagation(thetas=new_thetas, should_print=False)
    ...
    def classifyTestData(self, thetas, test_data):
        predictions = []

        for instance in test_data:
            prediction, activations = self.forwardPropagation(instance=instance, thetas=thetas, should_print=False)
            # prediction contem a probabilidade de pertencer a cada classe
            predicted_index_class = list(prediction).index(max(prediction))

            classes_list = list(instance.classification.keys())
            predicted_class = classes_list[predicted_index_class]
